...
<Tabs syncKey="package-manager">
  ...
  <TabItem label="pnpm">
    ...
    ```sh
    pnpm add effect-spacetimedb effect@4.0.0-beta.68 spacetimedb@2.5.0
    ```
  ...
  <TabItem label="bun">
    ...
    ```sh
    bun add effect-spacetimedb effect@4.0.0-beta.68 spacetimedb@2.5.0
    ```
...
4. Import `server-polyfills` once and build the native module export with
   `build(...)` from `effect-spacetimedb/server-compiler`.
...
If generated clients are committed, expect the CLI version header to include a
build commit hash that can differ by platform. Many repos normalize that header
and add `// @ts-nocheck` to generated TypeScript before checking drift; those
normalization steps belong in your repo tooling, not in the published package.
