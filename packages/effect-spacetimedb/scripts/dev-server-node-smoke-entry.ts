...
const program = Effect.gen(function* () {
  ...
  const session = yield* project(ExampleModule).client.ws.scopedGenerated({
    DbConnection: ExampleGeneratedClient.DbConnection,
    compression: "none",
...
}).pipe(Effect.scoped, Effect.timeout(Duration.seconds(120)))
