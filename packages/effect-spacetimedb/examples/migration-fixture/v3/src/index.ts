...
const account = Stdb.table("account", {
  ...
  columns: {
    ...
    archived: Stdb.bool().default(false),
  },
  indexes: (columns) => [
    Stdb.index("accountOwnerIdx", [columns.owner], { algorithm: "btree" }),
  ],
})
...
const AccountFunctions = Stdb.StdbGroup.make("Account")
  ...
  .add(
    Stdb.StdbFn.procedure("accountByOwner", {
      params: Stdb.struct({
        owner: String255,
      }),
...
const AccountFunctionsLive = Stdb.StdbBuilder.group(
  ...
  "Account",
  {
    ...
    accountByOwner: Effect.fn(function* ({ owner }) {
      const tx = yield* Tx
      return yield* tx.run(
        Effect.gen(function* () {
          const db = yield* Db
          const rows = yield* db.account.accountOwnerIdx.filterToArray(owner)
          return rows.map((row) => ({
            id: row.id ?? 0n,
            owner: row.owner,
