...
import { makeTransactionHelpers } from "./transaction-helpers.ts"
...
>(options: {
  ...
  const bindReducer = <Key extends ReducerKeys<Module>>(
    ...
    const spanOptions = makeHandlerSpanOptions("reducer")
    ...
    return {
      ...
      invoke: (ctx, args) => {
        if (disposed) {
          throw disposedError(String(key), "reducer")
        }
  ...
  const bindProcedure = <Key extends ProcedureKeys<Module>>(
    ...
    const spanOptions = makeHandlerSpanOptions("procedure")
    ...
    return {
      ...
      invoke: (ctx, args) => {
        if (disposed) {
          throw disposedError(String(key), "procedure")
        }
  ...
  const bindHttpHandler = <Key extends HttpHandlerKeys<Module>>(
    ...
    const spanOptions = makeHandlerSpanOptions("httpHandler")
      ...
      invoke: (ctx, req) => {
        ...
        try {
          ...
          const effect = provideHttpHandlerRuntime(
            ctx,
            txRunner,
            encodeHttpResult(spec)(handlerEffect).pipe(logHttpFailureCause),
  ...
  const bindView = <Key extends ViewKeys<Module>>(
    ...
    const spanOptions = makeHandlerSpanOptions("view")
    ...
    return {
      ...
      invoke: (ctx, args) => {
        if (disposed) {
          throw disposedError(String(key), "view")
        }
  ...
  const bindLifecycle = <Key extends LifecycleKeys, A, E>(
    ...
    const spanOptions = makeHandlerSpanOptions("lifecycle")
    ...
    return {
      ...
      invoke: (ctx) => {
        if (disposed) {
          throw disposedError(String(key), "lifecycle")
        }
  ...
  return withServerOwner(owner, server) as unknown as InternalServerInstance<
    Module,
    RuntimeR
  >
}
