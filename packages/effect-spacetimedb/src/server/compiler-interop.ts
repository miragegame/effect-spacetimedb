...
import type { AnyTypeBuilder } from "../contract/type/core.ts"
import {
  resetTypeBuilderCachesFor,
  seedNamedSatsTypeBuilder,
} from "../contract/type/fingerprint.ts"
import type { SatsTypeNameKind } from "../contract/type/name.ts"
...
import { Router, t, table } from "./host-abi-compiler.ts"
...
export const resetCompilerTypeBuilderCaches = (): void =>
  resetTypeBuilderCachesFor(t as never)

export const setCompilerTypeBuilderName = (
  builder: unknown,
  name: string,
): void => {
  ;(builder as { typeName?: string }).typeName = name
}

export const seedCompilerNamedTypeBuilder = (
...
export const toCompilerHttpHandlerCtx = <Module extends AnyModuleSpec>(
  ...
  ({
    ...
    withTx: <A>(body: (txCtx: { readonly db: unknown }) => A): A =>
      ctx.withTx(body),
