...
const hostLoggerLayer = Logger.layer([hostLogger])
...
export const makeConstrainedServerSupportLayer = (
    ...
    Layer.effectDiscard(
      Match.value(mode).pipe(
        Match.when("dev-guarded", () =>
          withServerPolyfills(installDevGuardsScoped),
        ),
        Match.when("runtime", () => installServerPolyfills),
        Match.exhaustive,
      ),
    ),
  )

export const makeConstrainedServerRuntimeLayer = (
  ctx: TimedRuntimeCtx,
  mode: ConstrainedServerRuntimeMode = defaultServerRuntimeMode,
) =>
  Layer.mergeAll(
    makeConstrainedServerSupportLayer(mode),
    Layer.succeed(Clock.Clock, makeServerClock(ctx)),
    Layer.succeed(Random.Random, makeServerRandom(ctx)),
