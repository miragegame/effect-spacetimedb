...
const requiredMetadata = <Key extends "item" | "lazy" | "values">(
  info: ValueTypeInfo,
  key: Key,
): NonNullable<ValueTypeInfo[Key]> => {
  const value: unknown = Reflect.get(info, key)
  if (!Object.hasOwn(info, key) || value == null) {
    throw new Error(
      `Malformed SpaceTimeDB ${info.kind} type metadata: missing ${key}`,
    )
  }
  return value as NonNullable<ValueTypeInfo[Key]>
}
  ...
  switch (info.kind) {
    ...
    case "array":
      return {
        _tag: "Array",
        kind: info.kind,
        item: requiredMetadata(info, "item"),
      }
    case "option":
      return {
        _tag: "Option",
    ...
    case "lazy":
      return {
        _tag: "Lazy",
        kind: info.kind,
        lazy: requiredMetadata(info, "lazy"),
      }
    case "literal":
      return {
        _tag: "Literal",
        kind: info.kind,
        values: requiredMetadata(info, "values"),
      }
    case "custom":
