...
const valueCodec = <A>(
  ...
  type: AnyValueType,
): Type.ValueCodec<A, unknown> =>
  Match.value(kind).pipe(
    Match.when("http", () => Type.httpCodec<A, unknown>(type)),
    Match.when("ws", () => Type.wsCodec<A, unknown>(type)),
    Match.when("db", () => Type.dbCodec<A, unknown>(type)),
    Match.exhaustive,
  )
