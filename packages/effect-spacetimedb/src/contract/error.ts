...
/** Builds a runtime guard for exactly the errors in a declared definition. */
export const declaredErrorGuard = <
  const Errors extends readonly TaggedErrorClass[],
>(
  definition: ErrorDefinition<Errors>,
) => Schema.is(definition.schema)
...
const declaredErrorDecodeFailure = (
  ...
  new StdbDecodeError({
    phase: "declaredError",
    cause: new Error(`${message}: ${String(readTaggedErrorTag(input) ?? "")}`),
...
const defineBase = <const Errors extends readonly TaggedErrorClass[]>(
  ...
  if (errorClasses.length === 0) {
    throw new Error("errors(...) requires at least one tagged error")
  }

  const tags = errorClasses.map((error) => tagOf(error))
  const duplicateTag = tags.find((tag, index) => tags.indexOf(tag) !== index)
  if (duplicateTag != null) {
    throw new Error(`errors(...) received duplicate error tag ${duplicateTag}`)
  }

  const reservedTag = tags.find((tag) => ReservedDeclaredErrorTags.has(tag))
  if (reservedTag != null) {
    throw new Error(
      `errors(...) cannot declare reserved wrapper error tag ${reservedTag}`,
    )
  }

  const unionSchema = Schema.Union(errorClasses) as Schema.Top
  ...
  const definition = {
    ...
    type,
    pick: (...pickedTags) => {
      const selected = errorClasses.filter((error) =>
        (pickedTags as ReadonlyArray<string>).includes(tagOf(error)),
      )

      if (selected.length !== pickedTags.length) {
        throw new Error(`errors(...).pick(...) received an unknown error tag`)
      }
...
export const errors = Object.assign(defineBase, { namespace, merge })
...
export const encodeString = <Definition extends AnyErrorDefinition>(
  ...
  Schema.encodeEffect(Schema.toCodecJson(definition.schema))(error).pipe(
    Effect.mapError((cause) =>
      declaredErrorSchemaFailure(cause, readTaggedErrorTag(error)),
    ),
    Effect.flatMap((encoded) => {
      const encodedTag = readTaggedErrorTag(encoded)
      const runtimeTag = readTaggedErrorTag(error)
      if (encodedTag == null) {
        return Effect.fail(
...
export const decodeString = <Definition extends AnyErrorDefinition>(
  ...
  Schema.decodeUnknownEffect(TaggedErrorEnvelopeStringSchema)(value).pipe(
    ...
    Effect.flatMap((envelope) => {
      ...
      const innerTag = readTaggedErrorTag(envelope.error)
