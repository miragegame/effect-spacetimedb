...
import * as TypeSats from "./contract/type/sats.ts"
  ...
  return ValueType.attachStdbType(
    ...
    (factories) =>
      factories.result(
        returnsWireUnit
          ? factories.unit()
          : TypeSats.typeBuilderWithFactories(returns, factories),
        TypeSats.typeBuilderWithFactories(definition.type, factories),
      ),
    {
      kind: "result",
