...
export const messageFromUnknown = (cause: unknown): string | undefined => {
  ...
  if (typeof cause === "object" && cause !== null) {
    const rendered = Inspectable.toStringUnknown(cause, 0)
    const limit = 2048
...
export const encodeArgsArray = <A>(
  ...
  encodeArgs(type, value).pipe(
    Effect.flatMap((encoded) => {
      ...
      return Effect.try({
        try: () =>
          encodedStructEntries(fields, encoded).map(
...
// Public root wrapper: callers get params/value type agreement. The looser
// encodeArgsArray stays in /testing for codec fixtures that intentionally pass
// unknown values through the encoder.
export const encodeCallArgs = <Params extends Type.StructLikeValueType>(
  paramsType: Params,
  value: Type.TypeOf<Params>,
): Effect.Effect<ReadonlyArray<unknown>, StdbDecodeError> =>
  encodeArgsArray(paramsType, value)

export const remoteRejectedFromRaw = (
  raw: string,
  status?: number,
): RemoteRejectedError => {
