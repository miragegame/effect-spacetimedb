...
import {
  isMissingOrUnitWireValue as isSatsUnitValue,
  singleEntryEnvelopeOf,
  taggedEnvelopeOf,
} from "../../contract/type/envelope.ts"
export { isMissingOrUnitWireValue as isSatsUnitValue } from "../../contract/type/envelope.ts"
...
export const normalizeResultValue = (
  ...
  if (envelope !== undefined) {
    const isExactEnvelope = envelope.keys.length === (envelope.hasValue ? 2 : 1)
    if (!isExactEnvelope) {
      return value
    }
...
export const normalizeSumValue = (
  ...
  const envelope = taggedEnvelopeOf(value)
  if (envelope !== undefined) {
    const variant = variants[envelope.tag]
    const isExactEnvelope = envelope.keys.length === (envelope.hasValue ? 2 : 1)
  ...
  const singleEntry = singleEntryEnvelopeOf(value)
  if (singleEntry !== undefined) {
    const variant = variants[singleEntry.tag]
    if (variant != null) {
      return normalizeSumVariant(
        singleEntry.tag,
        variant,
