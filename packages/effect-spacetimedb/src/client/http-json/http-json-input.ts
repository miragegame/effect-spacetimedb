...
import { singleEntryEnvelopeOf } from "../../contract/type/envelope.ts"
...
export const prepareSumInputValue = (
  ...
  const entry = singleEntryEnvelopeOf(value)
  if (entry === undefined) return value

  const variant = variants[entry.tag]
  if (variant === undefined) return value

  return {
    [entry.tag]: isUnitValueType(variant)
      ? entry.value
      : recurse(variant, entry.value),
  }
}
...
export const prepareHttpInputValue = foldValue<unknown>({
  ...
  option: (_type, item, value, recurse) => {
    const optionEntry = singleEntryEnvelopeOf(value)
    if (optionEntry?.tag !== "some") return value
    return {
      some: recurse(item, optionEntry.value),
    }
  },
