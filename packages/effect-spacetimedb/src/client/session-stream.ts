...
const TableStreamOverflowErrorTypeId = errorTypeId("TableStreamOverflowError")
export class TableStreamOverflowError extends Data.TaggedError(
  "TableStreamOverflowError",
)<{
  readonly bufferSize: number
}> {
  readonly [TableStreamOverflowErrorTypeId] = TableStreamOverflowErrorTypeId
  static is = hasErrorTypeId<TableStreamOverflowError>(
    TableStreamOverflowErrorTypeId,
  )

  override get message(): string {
    return `Table delta stream buffer capacity ${this.bufferSize.toString()} was exceeded`
  }
}

// Snapshot signals stay bounded + sliding: consumers re-read authoritative
// snapshots, so overflow is recoverable.
...
const DELTA_STREAM_BUFFER = {
  bufferSize: 1024,
  strategy: "suspend",
} satisfies Required<RuntimeSessionStreamBufferOptions>

const tableOverflowOptions = (
  strategy: Required<RuntimeSessionStreamBufferOptions>["strategy"],
) =>
  Match.value(strategy).pipe(
    Match.when("suspend", () => ({
      overflowFailure: (bufferSize: number) =>
        new SubscriptionTransportError({
          cause: new TableStreamOverflowError({ bufferSize }),
        }),
    })),
    Match.when("sliding", () => ({})),
    Match.when("dropping", () => ({})),
...
const deltaStreamBuffer = (
  buffer: SessionStreamBufferOptions | undefined,
): Required<RuntimeSessionStreamBufferOptions> => ({
  bufferSize: buffer?.bufferSize ?? DELTA_STREAM_BUFFER.bufferSize,
  strategy: buffer?.strategy ?? DELTA_STREAM_BUFFER.strategy,
})
...
const sessionStream = <A>(options: {
  ...
  readonly subscribe?: (
    onFailure: (failure: SubscriptionFailure) => void,
      ...
      Effect.andThen(
        Effect.gen(function* () {
          ...
          if (options.subscribe != null) {
            yield* options.subscribe(emit.fail)
          }
...
export const streamTableChanges = <Row, MappedRow = Row, Ctx = unknown>(
  ...
  const streamBuffer = deltaStreamBuffer(buffer)
  ...
  return sessionStream<TableChange<MappedRow>>({
    ...
    buffer: streamBuffer,
    ...tableOverflowOptions(streamBuffer.strategy),
  })
}
