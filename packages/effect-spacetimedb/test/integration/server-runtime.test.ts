  ...
  Effect.acquireRelease(
    Effect.try({
      try: () => {
        ...
        "effect-spacetimedb/integration/server-runtime",
      ),
    }).pipe(Effect.orDie),
    (original) =>
      Effect.try({
...
const harnessByDb = new WeakMap<
  object,
  StdbTesting.TestModuleHarness<typeof FullModule>
>()

const harnessOf = (db: ReturnType<typeof makeFakeDb>) =>
  Option.fromUndefinedOr(harnessByDb.get(db)).pipe(Option.getOrThrow)

const makeProcedureCtx = (db: ReturnType<typeof makeFakeDb>) =>
  harnessOf(db).makeProcedureCtx({
    sender: "sender" as never,
    databaseIdentity: "database" as never,
    identity: "sender" as never,
    timestamp: { microsSinceUnixEpoch: 1000n } as never,
...
describe("server runtime", (it) => {
  it.effect("rejects malformed raw db shapes before entering a handler", () =>
    Effect.gen(function* () {
      let handlerEntered = false
      let rawTableTouched = 0
      const malformedDb = Object.defineProperty({}, "user", {
        enumerable: true,
        get: () => {
          rawTableTouched = rawTableTouched + 1
          return {}
        },
      })
      ...
      expect(() =>
        reducers.userUpsert!.invoke(
          { ...makeReducerCtx(makeFakeDb([])), db: malformedDb } as never,
          { userId: "user-1" as never, name: "Ada" as never },
        ),
      ).toThrow(StdbDbSchemaMismatchError)
      expect(handlerEntered).toBe(false)
      expect(rawTableTouched).toBe(0)
    }),
  )
  ...
  it.effect("rejects external calls to scheduled compiler exports", () =>
    Effect.gen(function* () {
      ...
      const externalCtx = {
        ...
        databaseIdentity: new Identity(1n),
        identity: new Identity(1n),
      }
      const rawArgs = {
        data: {
  ...
  it.effect("uses connection-owned cache and onApplied-only WS readiness", () =>
    Effect.gen(function* () {
      const rows = [makeUserRow("user-1", "Ada")]
      const relation = Object.assign(makeStaticRelationHandle(rows), {
        id: {
          find: (id: UserRow["id"]) =>
