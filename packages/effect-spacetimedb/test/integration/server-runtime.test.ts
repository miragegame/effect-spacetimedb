...
describe("server runtime", (it) => {
  ...
  it.effect("interrupts rejected suspended reducer fibers", () =>
    Effect.gen(function* () {
      ...
      yield* waitForPredicate(
        () => interrupted,
        "Expected suspended reducer fiber to be interrupted",
      )
  ...
  it.effect("uses connection-owned cache and onApplied-only WS readiness", () =>
    Effect.gen(function* () {
      ...
      let appliedCallback: (() => void) | undefined
      ...
      yield* waitForPredicate(
        () => typeof appliedCallback === "function",
        "ws stream did not register onApplied",
      )
      ...
      expect(typeof appliedCallback).toBe("function")
