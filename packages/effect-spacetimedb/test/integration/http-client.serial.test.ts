...
describe("http client", (it) => {
  it.effect(
    "retains non-2xx status for reducer and procedure rejections",
    () => {
      const Full = Stdb.project(FullModule)
      const client = Full.client.http.make(HttpConfig)
      const reject = makeMockHttpClientLayer(() =>
        Effect.succeed({ status: 429, body: "rate limited" }),
      )

      return Effect.gen(function* () {
        const reducerFailure = yield* client.reducers
          .userUpsert({
            userId: "user-1" as never,
            name: "Ada" as never,
          })
          .pipe(Effect.provide(reject), Effect.flip)
        const procedureFailure = yield* client.procedures
          .userGet({ userId: "user-1" as never })
          .pipe(Effect.provide(reject), Effect.flip)

        expect(reducerFailure).toBeInstanceOf(StdbTesting.RemoteRejectedError)
        expect(procedureFailure).toBeInstanceOf(StdbTesting.RemoteRejectedError)
        expect(reducerFailure).toMatchObject({
          status: 429,
          raw: "rate limited",
        })
        expect(procedureFailure).toMatchObject({
          status: 429,
          raw: "rate limited",
        })
      })
              ...
              makeMockHttpClientLayer((request) =>
                Effect.try({
                  try: () => {
                    ...
                    "effect-spacetimedb/integration/http-client",
                  ),
                }),
              ),
            ),
            ...
            makeMockHttpClientLayer((request) =>
              Effect.try({
                try: () => {
                  ...
                  "effect-spacetimedb/http-client",
                ),
              }),
            ),
          ),
      ...
      return Effect.gen(function* () {
        ...
        const mockLayer = makeMockHttpClientLayer((request) =>
          Effect.try({
            ...
            catch: testEffectCallbackError("effect-spacetimedb/http-client"),
          }),
        )
