...
type ExampleGeneratedErrorContext = GeneratedErrorContextOf<
  typeof ExampleGeneratedClient.DbConnection
>
...
export type LiveHarness = {
  ...
  readonly status: PublishedModuleRuntime["status"]
  readonly reset: PublishedModuleRuntime["reset"]
  readonly republish: PublishedModuleRuntime["republish"]
  readonly generatedClient: typeof ExampleGeneratedClient
  readonly makeWsConfig: <Module extends AnyModuleSpec>(
    module: Module,
  ) => WsBuilderConfig<Module, ExampleGeneratedErrorContext>
...
const asLiveConnection = <Module extends AnyModuleSpec, ErrorContext>(
  _module: Module,
  connection: ManagedWsConnection<Module, ErrorContext, unknown>,
): LiveConnection<Module> => connection as LiveConnection<Module>

export const typedConnection = <Module extends AnyModuleSpec, ErrorContext>(
  session: {
    readonly connection: ManagedWsConnection<Module, ErrorContext, unknown>
  },
  module: Module,
...
const makeLiveHarness = Effect.fn(function* () {
  ...
  const makeWsConfigWithToken = <Module extends AnyModuleSpec>(
    ...
    const config: WsGeneratedConfig<
      Module,
      typeof generatedClient.DbConnection
    > = {
    ...
    return ClientGeneratedWsAdapter.generatedConfig(config)
  }
  ...
  const makeConnection = <Module extends AnyModuleSpec>(module: Module) =>
    makeScopedFromModulePlan({
      plan: makeModulePlan(module),
      config: makeWsConfig(module),
    }).pipe(
  ...
  const makeAnonymousConnection = <Module extends AnyModuleSpec>(
    ...
    makeAnonymousIdentityToken(runtime.baseUrl).pipe(
      ...
      Effect.flatMap((token) =>
        makeScopedFromModulePlan({
          plan: makeModulePlan(module),
          config: makeWsConfigWithToken(module, token),
        }),
      ),
      Effect.map((session) => typedConnection(session, module)),
      Effect.mapError((cause) => new LiveHarnessConnectionError({ cause })),
...
export const callLiveReducer = (
  ...
  Effect.tryPromise({
    ...
    catch: testEffectCallbackError("effect-spacetimedb/helpers/live-harness"),
  })
