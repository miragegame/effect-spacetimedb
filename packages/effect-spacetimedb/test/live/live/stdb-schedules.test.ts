...
const { Example: Live, ExampleModule: LiveModule } = ExampleModuleFixture
const wireFunction = (name: string) => liveFunctionName(LiveModule, name)
type LiveSubscriptionBuilder = ReturnType<
  TypedLiveConnection<typeof LiveModule>["subscriptionBuilder"]
>

const observeSubscriptionApplies = (
  connection: TypedLiveConnection<typeof LiveModule>,
) => {
  let appliedCount = 0
    ...
    () =>
      provideLiveTest(
        Effect.gen(function* () {
          ...
          const generatedClient = live.generatedClient as {
            readonly procedures?: Record<string, unknown>
          }
          const generatedProcedures = generatedClient.procedures ?? {}
          expect("reminderFireProcedure" in generatedProcedures).toBe(false)
        }),
      ),
    180_000,
  )
