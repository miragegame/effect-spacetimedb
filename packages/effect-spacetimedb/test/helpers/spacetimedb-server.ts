// lint-ignore: unused-files - resolved through the unit Vitest alias for spacetimedb/server.
// lint-ignore: unused-exports - exports model the spacetimedb/server package surface consumed through a Vitest alias.
...
class TestSchema {
  ...
  view(
    options: ViewOptions,
    _returnType: unknown,
    _handler: (ctx: unknown) => unknown,
  ): ModuleExport {
    return viewModuleExport(options)
  }
...
const viewModuleExport = (options: ViewOptions): ModuleExport => ({
  [registerExport]: (schema, exportName) => {
...
export const isRowTypedQuery = (): boolean => false
