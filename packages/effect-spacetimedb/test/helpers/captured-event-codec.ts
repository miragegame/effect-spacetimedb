...
export const deserializeCapturedValue = (value: CapturedJson): unknown => {
  ...
  switch (captureType(record)) {
    ...
    case "Error": {
      ...
      error.name = ownString(record, "name") ?? "Error"
      if (
        typeof record.properties === "object" &&
        record.properties !== null &&
        !Array.isArray(record.properties)
...
export const readCapturedJson = async (url: URL): Promise<unknown> =>
  deserializeCapturedValue(await Bun.file(url).json())

export const writeCapturedJson = async (
  url: URL,
  value: unknown,
  ...
  await Bun.write(url, `${JSON.stringify(captured, null, 2)}\n`)
}
