...
describe("module validation diagnostics", (it) => {
  it.effect("rejects invalid and reserved group client property keys", () =>
    Effect.gen(function* () {
      const InvalidModule = {
        kind: "module" as const,
        name: "invalid_group_keys",
        settings: {},
        tables: {},
        views: {},
        reducers: {},
        ...
        httpHandlers: {},
        reducerGroups: {
          reducerEndpoint: "reducers",
          subscriptionEndpoint: "subscribe",
        },
        procedureGroups: {
          procedureEndpoint: "constructor",
        },
        httpGroups: {
          httpEndpoint: "not-valid",
        },
        lifecycle: {},
        diagnostics: [],
      }
      ...
      expect(Stdb.validate(withWireNames(InvalidModule))).toEqual([
        expect.objectContaining({
          code: "InvalidGroupClientKey",
          path: ["reducerGroups", "reducerEndpoint"],
          severity: "error",
        }),
        expect.objectContaining({
          code: "InvalidGroupClientKey",
          path: ["reducerGroups", "subscriptionEndpoint"],
          severity: "error",
        }),
        expect.objectContaining({
          code: "InvalidGroupClientKey",
          path: ["procedureGroups", "procedureEndpoint"],
          severity: "error",
        }),
        expect.objectContaining({
          code: "InvalidGroupClientKey",
          path: ["httpGroups", "httpEndpoint"],
          severity: "error",
        }),
      ])
    }),
        ...
        try: () => DuplicateViewsModule.spec,
          ...
          cause instanceof Stdb.StdbValidationError
            ...
            : new TestEffectCallbackError({
                operation: "effect-spacetimedb/unit/module-validation",
                cause,
              }),
      }).pipe(
  ...
  it("rejects group ids that are not valid client keys", () => {
    expect(
      () =>
        Stdb.StdbModule.make("invalid_group_client_key", {}).add(
  ...
  it.effect("reports shared unsupported descriptors at the first path", () =>
    Effect.gen(function* () {
      ...
      const unsupported = Stdb.validate(withWireNames(InvalidModule)).filter(
        (diagnostic) => diagnostic.code === "UnsupportedTypeDescriptor",
      )
  ...
  it.effect("memoizes structural validation across callable roots", () =>
    Effect.gen(function* () {
      const raw = Schema.instanceOf(URL) as unknown as Stdb.AnyValueType
      const SharedModule = {
        kind: "module" as const,
        name: "shared_callable_validation",
        settings: {},
        tables: {},
        views: {},
        reducers: {
          first_reducer: Stdb.StdbFn.reducer("first_reducer", {
            params: Stdb.struct({ shared: raw }),
          }).spec,
          second_reducer: Stdb.StdbFn.reducer("second_reducer", {
            params: Stdb.struct({ shared: raw }),
          }).spec,
        },
        procedures: {},
      ...
      const unsupported = Stdb.validate(withWireNames(SharedModule)).filter(
        (diagnostic) => diagnostic.code === "UnsupportedTypeDescriptor",
      )
      expect(unsupported).toEqual([
        expect.objectContaining({
          path: ["reducers", "first_reducer", "params", "shared"],
        }),
      ])
    }),
  )

  it.effect("memoizes canonical validation across callable roots", () =>
    Effect.gen(function* () {
      const sharedParams = Stdb.struct({
        fooBar: Stdb.string(),
        foo_bar: Stdb.string(),
      })
      const SharedModule = {
        kind: "module" as const,
        name: "shared_canonical_validation",
        settings: {},
        tables: {},
        views: {},
        reducers: {
      ...
      const collisions = Stdb.validate(withWireNames(SharedModule)).filter(
        (diagnostic) => diagnostic.code === "CanonicalNameCollision",
      )
      expect(collisions).toEqual([
        expect.objectContaining({
          path: ["reducers", "first_reducer", "params"],
        }),
      ])
    }),
  )
