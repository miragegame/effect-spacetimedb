...
describeLayer("transport tag-casing goldens", (it) => {
  it.effect("keeps literal enum casing exact across transports", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* StdbTesting.ClientValueCodec.ws.encode(Literal, "joined"),
      ).toEqual({
        tag: "Joined",
      })
      expect(
        yield* StdbTesting.ClientValueCodec.db.encode(Literal, "joined"),
      ).toEqual({
        tag: "joined",
      })
      expect(
        yield* StdbTesting.ClientValueCodec.http.encode(Literal, "joined"),
      ).toEqual({
        tag: "joined",
      })
    }),
