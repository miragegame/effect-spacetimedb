...
const expectNativeHostTransportRoundTrip = Effect.fn(function* <A>(
  type: Parameters<typeof StdbTesting.ContractType.typeBuilderWithFactories>[0],
  value: A,
) {
  const wsEncoded = yield* StdbTesting.ClientValueCodec.ws.encode(type, value)
  const wsNative = nativeRoundTrip(type, wsEncoded)
  expect(yield* StdbTesting.ClientValueCodec.ws.decode(type, wsNative)).toEqual(
    value,
  )

  const dbEncoded = yield* StdbTesting.ClientValueCodec.db.encode(type, value)
  const dbNative = nativeRoundTrip(type, dbEncoded)
  expect(yield* StdbTesting.ClientValueCodec.db.decode(type, dbNative)).toEqual(
    value,
  )
})
...
describe("transport codec boundary", (it) => {
  it.effect("round-trips every TypeKind through WS and DB codecs", () =>
    Effect.forEach(
      ...
      ([kind, sample]) =>
        Effect.gen(function* () {
          const wsEncoded = yield* StdbTesting.ClientValueCodec.ws.encode(
            sample.type,
            sample.value,
          )
          const dbEncoded = yield* StdbTesting.ClientValueCodec.db.encode(
            sample.type,
            sample.value,
          )

          expect(
            yield* StdbTesting.ClientValueCodec.ws.decode(
              sample.type,
              wsEncoded,
            ),
          ...
          expect(
            yield* StdbTesting.ClientValueCodec.db.decode(
              sample.type,
              dbEncoded,
            ),
  ...
  it.effect("normalizes HTTP JSON using authored Stdb type metadata", () =>
    Effect.gen(function* () {
      ...
      expect(
        StdbTesting.ClientValueCodec.httpJson.encodeInput([decoded.id]),
  ...
  it.effect("routes DB values through the explicit DB codec boundary", () =>
    Effect.gen(function* () {
      ...
      const encoded = yield* StdbTesting.ClientValueCodec.db.encode(Row, {
        id: 1n,
      })
  ...
  it.effect("encodes option WS params as native bare values", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* StdbTesting.ClientValueCodec.ws.encode(
          PresenceSession,
          "session-1",
        ),
      ).toBe("session-1")
      ...
      expect(
        yield* StdbTesting.ClientValueCodec.http.encode(
          PresenceSession,
          "session-1",
        ),
  ...
  it.effect("encodes unbounded bigint values as native strings", () =>
    Effect.gen(function* () {
      ...
      const encoded = yield* StdbTesting.ClientValueCodec.ws.encode(
        Count,
        9007199254740993n,
      )
      ...
      expect(yield* StdbTesting.ClientValueCodec.ws.decode(Count, native)).toBe(
        9007199254740993n,
      )
    }),
  )
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const decoded = yield* StdbTesting.ClientValueCodec.db.decode(Row, {
          state: {
            tag: "Active",
            value: {},
          },
  ...
  it.effect(
    "decodes generated SDK unit result payloads as authored result values",
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* StdbTesting.ClientValueCodec.db.decode(Result, {
            tag: "ok",
            value: {},
          }),
        ).toEqual({
  ...
  it.effect("rejects non-unit generated SDK enum payload values", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* Effect.exit(
        StdbTesting.ClientValueCodec.db.decode(Row, {
          state: {
            tag: "Active",
            value: {
              unexpected: true,
  ...
  it.effect("decodes generated SDK PascalCase sum tags as authored tags", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* StdbTesting.ClientValueCodec.db.decode(Content, {
          tag: "Prose",
          value: {
            text: "hello",
          },
  ...
  it.effect("round-trips byte arrays through HTTP JSON", () =>
    Effect.gen(function* () {
      ...
      expect(StdbTesting.ClientValueCodec.httpJson.encodeInput(value)).toBe(
        '{"data":[0,1,255]}',
      )
      expect(
        StdbTesting.ClientValueCodec.httpJson.encodeInput({
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          StdbTesting.ClientValueCodec.httpJson.encodeInput([decoded.u256]),
        ).toBe(
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* StdbTesting.ClientValueCodec.db.encode(
            Status,
            "pending_review",
          ),
        ).toEqual({
        ...
        expect(
          yield* StdbTesting.ClientValueCodec.db.decode(Status, {
            tag: "pending_review",
          }),
        ).toBe("pending_review")
        expect(
        ...
        expect(
          yield* StdbTesting.ClientValueCodec.http.decode(Status, {
            tag: "PendingReview",
          }),
        ).toBe("pending_review")
        expect(
          yield* StdbTesting.ClientValueCodec.db.encode(Action, "edit-action"),
        ).toEqual({
          tag: "EditAction",
        })
        expect(
        ...
        expect(
          yield* StdbTesting.ClientValueCodec.ws.encode(Action, "edit-action"),
        ).toEqual({
          tag: "EditAction",
        })
        expect(
          yield* StdbTesting.ClientValueCodec.db.decode(Action, {
            tag: "edit-action",
          }),
        ).toBe("edit-action")
        expect(
          yield* StdbTesting.ClientValueCodec.http.decode(Action, {
            tag: "EditAction",
          }),
        ).toBe("edit-action")
        expect(
  ...
  it.effect("rejects malformed SATS option tuple variants", () =>
    Effect.gen(function* () {
      ...
      const badVariant = yield* Effect.exit(
        StdbTesting.ClientValueCodec.httpJson.decodeOutput(Option, "[2,[]]"),
      )
      const badNonePayload = yield* Effect.exit(
        StdbTesting.ClientValueCodec.httpJson.decodeOutput(Option, "[1,123]"),
      )
  ...
  it.effect("rejects malformed SATS none objects", () =>
    Effect.gen(function* () {
      ...
      const decoded = yield* StdbTesting.ClientValueCodec.httpJson.decodeOutput(
        Option,
        '{"none":{}}',
      )
      const badNonePayload = yield* Effect.exit(
  ...
  it.effect("rejects malformed unit result payloads", () =>
    Effect.gen(function* () {
      ...
      const badTuple = yield* Effect.exit(
        StdbTesting.ClientValueCodec.httpJson.decodeOutput(Envelope, "[0,123]"),
      )
  ...
  it.effect("validates exact result schema envelopes", () =>
    Effect.gen(function* () {
      const Result = StdbTesting.ContractType.result(
        StdbTesting.ContractType.string(),
        StdbTesting.ContractType.unit(),
      ...
      expect(
        yield* Schema.decodeUnknownEffect(Result.schema)({
          tag: "ok",
          value: "accepted",
        }),
      ).toEqual({ ok: "accepted" })
      expect(
        yield* Schema.decodeUnknownEffect(Result.schema)({ tag: "err" }),
      ...
      expect(
        yield* Schema.encodeEffect(Result.schema)({ ok: "accepted" }),
      ).toEqual({
        tag: "ok",
        value: "accepted",
      })
      expect(
        yield* Schema.encodeEffect(Result.schema)({ err: undefined }),
      ).toEqual({
        tag: "err",
      })

      const malformedResults = yield* Effect.forEach(
        [
          [],
          { ok: "accepted" },
          { tag: "ok" },
          { tag: "ok", value: "accepted", extra: true },
          { tag: 0, value: "accepted" },
          { tag: "missing", value: "accepted" },
        ],
        (value) =>
          Effect.exit(Schema.decodeUnknownEffect(Result.schema)(value)),
      )
      const ambiguousEncode = yield* Effect.exit(
        Schema.encodeEffect(Result.schema)({
          ok: "accepted",
          err: undefined,
        } as never),
      )
      const extraEncode = yield* Effect.exit(
        Schema.encodeEffect(Result.schema)({
          ok: "accepted",
          extra: true,
        } as never),
      ...
      expect(malformedResults.every(Exit.isFailure)).toBe(true)
      expect(Exit.isFailure(ambiguousEncode)).toBe(true)
      expect(Exit.isFailure(extraEncode)).toBe(true)
    }),
  )

  it.effect("validates exact sum schema envelopes", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* Schema.decodeUnknownEffect(Sum.schema)(["active", "on"]),
      ).toEqual({
        tag: "active",
        value: "on",
      })
      expect(
        yield* Schema.decodeUnknownEffect(Sum.schema)(["deleted"]),
      ).toEqual({
        tag: "deleted",
      })
      expect(
      ...
      expect(
        yield* Schema.encodeEffect(Sum.schema)(Sum.make.active("on")),
      ).toEqual({
        active: "on",
      })
      expect(yield* Schema.encodeEffect(Sum.schema)(Sum.make.deleted)).toEqual({
        deleted: {},
      })

      const malformedSums = yield* Effect.forEach(
        [
          ["active", "on", true],
          [0, "on"],
          ["active"],
          ...
          { tag: "deleted", value: { extra: true } },
          { tag: "active" },
        ],
        (value) => Effect.exit(Schema.decodeUnknownEffect(Sum.schema)(value)),
      )
      const missingValueEncode = yield* Effect.exit(
        Schema.encodeEffect(Sum.schema)({ tag: "active" } as never),
      )
      const extraEncode = yield* Effect.exit(
        Schema.encodeEffect(Sum.schema)({
          tag: "deleted",
          extra: true,
        } as never),
      )

      expect(malformedSums.every(Exit.isFailure)).toBe(true)
      expect(Exit.isFailure(missingValueEncode)).toBe(true)
      expect(Exit.isFailure(extraEncode)).toBe(true)
    }),
  )

  it.effect("validates struct optional and required field wire behavior", () =>
    Effect.gen(function* () {
      const Row = StdbTesting.ContractType.struct({
      ...
      expect(
        yield* Schema.decodeUnknownEffect(Row.schema)({
          id: "user-1",
        }),
      ).toEqual({
        id: "user-1",
        maybeCount: undefined,
      })
      expect(
        yield* Schema.decodeUnknownEffect(Row.schema)({
          id: "user-1",
      ...
      expect(
        yield* Schema.encodeEffect(Row.schema)({
          id: "user-1",
        } as never),
      ).toEqual({
        id: "user-1",
      ...
      const nonObject = yield* Effect.exit(
        Schema.decodeUnknownEffect(Row.schema)(123),
      )
      const missingRequired = yield* Effect.exit(
        Schema.decodeUnknownEffect(Row.schema)({
          maybeCount: { none: {} },
        }),
      )
      const missingRequiredEncode = yield* Effect.exit(
        Schema.encodeEffect(Row.schema)({
          maybeCount: undefined,
        } as never),
      )

      expect(Exit.isFailure(nonObject)).toBe(true)
      expect(Exit.isFailure(missingRequired)).toBe(true)
      expect(Exit.isFailure(missingRequiredEncode)).toBe(true)
    }),
  )
})
