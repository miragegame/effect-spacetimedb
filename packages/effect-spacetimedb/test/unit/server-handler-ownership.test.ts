...
const makeHandlers = (server: StdbTesting.ServerInstance<typeof FullModule>) =>
  server.handlers({
    reducers: {
...
describe("server handler ownership", (it) => {
  it.effect("accepts same-instance handler bundles", () =>
    Effect.gen(function* () {
      ...
      expect(() =>
        StdbTesting.assertOwnedHandlerBundle(
          server[StdbTesting.ServerOwnerSymbol],
          handlers,
        ),
      ).not.toThrow()
    }),
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(() => firstServer.handlers(mismatchedHandlers)).toThrow(
          "Server handlers must be assembled by the same Server.make(...) instance as the provided server",
        )

        expect(() =>
          StdbTesting.assertOwnedHandlerBundle(
            firstServer[StdbTesting.ServerOwnerSymbol],
            mismatchedHandlers,
          ),
        ).toThrow(
          "Server handlers must be assembled by the same Server.make(...) instance as the provided server",
