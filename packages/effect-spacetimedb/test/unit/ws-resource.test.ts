...
describe("ws resource", (it) => {
  ...
  it.effect("uses stable named ws session keys per projected module", () =>
    Effect.gen(function* () {
      ...
      expect(full.client.ws.tag().key).toBe(
        'effect-spacetimedb/Client/WsSession/["array",[["string","example"],["string","default"]]]',
      )
      expect(full.client.ws.tag("main").key).toBe(
        'effect-spacetimedb/Client/WsSession/["array",[["string","example"],["string","main"]]]',
      )
      expect(full.client.ws.tag("main").key).toBe(
      ...
      const first = StdbTesting.project(
        Stdb.StdbModule.make("a/b", {}).spec,
      ).client.ws.tag("c")
      const second = StdbTesting.project(
        Stdb.StdbModule.make("a", {}).spec,
      ).client.ws.tag("b/c")
  ...
  it.effect(
    "keeps typed ws session tags on the same runtime key as the convenience tag",
    () =>
      Effect.gen(function* () {
        const full = StdbTesting.project(FullModule)

        type GeneratedErrorContext = {
          readonly generatedContext: "typed"
        }

        expect(full.client.ws.tag<GeneratedErrorContext>().key).toBe(
          full.client.ws.Session.key,
        )
        expect(full.client.ws.tag<GeneratedErrorContext>("main").key).toBe(
          full.client.ws.tag("main").key,
        )
      }),
  )
