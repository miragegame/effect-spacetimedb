...
import { Range as NativeRange } from "spacetimedb/server"
import {
...
class TestNativeRange<Value> {
  constructor(
    readonly from: NativeBound<Value>,
    readonly to: NativeBound<Value>,
  ) {}
}

class TestForeignRange<Value> {
  constructor(
    readonly from: NativeBound<Value>,
    readonly to: NativeBound<Value>,
  ) {}
...
const LazyHandleModule = Stdb.StdbModule.make("lazy_handles", {}).addTables(
  Stdb.table("lazyA", { columns: { value: Stdb.string() } }),
  Stdb.table("lazyB", { columns: { value: Stdb.string() } }),
).spec
...
describe("db handle factory", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        let receivedScoreRange: unknown
        const scoreFilter = rawDb.algorithmUser.scoreDirect.filter
        rawDb.algorithmUser.scoreDirect.filter = (value: unknown) => {
          receivedScoreRange = value
          return scoreFilter(value)
        }
        const db = factory.readwrite(rawDb as never)
        const readonlyDb = factory.readonly(rawDb as never)
        ...
        expect(receivedScoreRange).toBeInstanceOf(NativeRange)
        expect(receivedScoreRange).toMatchObject(scoreRange)

        class StructuralScoreRange {
          constructor(
            readonly from: {
        ...
        const structuralRange = new StructuralScoreRange(
          { tag: "unbounded" },
          { tag: "included", value: 20 },
        )
        expect(
          yield* db.algorithmUser.scoreDirect.filterToArray(structuralRange),
        ).toEqual([algorithmAdaRow, algorithmBeaRow])
        expect(receivedScoreRange).toBeInstanceOf(NativeRange)
        expect(receivedScoreRange).toMatchObject({
          from: { tag: "unbounded" },
          to: { tag: "included", value: 20 },
        })
        expect(
  ...
  it.effect("encodes structural server ranges as native prefix bounds", () =>
    Effect.gen(function* () {
      const factory = StdbTesting.makeDbHandleFactory(IndexedModule, {
        makeRange: (from, to) => new TestNativeRange(from, to),
      })
      ...
      expect(Array.isArray(receivedRange)).toBe(true)
      const receivedTenantRange = Array.isArray(receivedRange)
        ? receivedRange[0]
        : undefined
      expect(receivedTenantRange).toBeInstanceOf(TestNativeRange)
      if (receivedTenantRange instanceof TestNativeRange) {
        expect(receivedTenantRange.from).toEqual(tenantRange.from)
        expect(receivedTenantRange.to).toEqual(tenantRange.to)
      }

      const foreignTenantRange = new TestForeignRange(
        tenantRange.from,
        tenantRange.to,
      )
      expect(
        yield* db.user.tenantName.filterToArray({
          tenant: foreignTenantRange,
        }),
      ).toEqual([])
      const convertedForeignRange = Array.isArray(receivedRange)
        ? receivedRange[0]
        : undefined
      expect(convertedForeignRange).toBeInstanceOf(TestNativeRange)
      expect(convertedForeignRange).not.toBeInstanceOf(TestForeignRange)
      if (convertedForeignRange instanceof TestNativeRange) {
        expect(convertedForeignRange.from).toEqual(tenantRange.from)
        expect(convertedForeignRange.to).toEqual(tenantRange.to)
      }
    }),
  )
    ...
    Effect.gen(function* () {
      ...
      const nextFailure = failureFromExit(
        yield* Effect.exit(
          Effect.try({
            try: () => iterator.next(),
            catch: testEffectCallbackError("effect-spacetimedb/unit/db-handle"),
          }),
        ),
      )
  ...
  it.effect("first decodes one row and always closes the iterator", () =>
    Effect.gen(function* () {
      const factory = StdbTesting.makeDbHandleFactory(IndexedModule)
      let nextCount = 0
      let returnCount = 0
      const iterator = {
        next: () => {
          ...
          return { done: false, value: nextCount === 1 ? adaRow : bobRow }
        },
        return: () => {
          returnCount = returnCount + 1
      ...
      const rawDb = makeRawDb([adaRow, bobRow])
      Object.assign(rawDb.user, { iter: () => iterator })
      const db = factory.readwrite(rawDb as never)
  ...
  it.effect("first closes the iterator when row decoding fails", () =>
    Effect.gen(function* () {
      const factory = StdbTesting.makeDbHandleFactory(IndexedModule)
      let returnCount = 0
      const rawDb = makeRawDb([])
      Object.assign(rawDb.user, {
        iter: () => ({
          next: () => ({ done: false, value: { id: 1 } }),
          return: () => {
            returnCount = returnCount + 1
            return { done: true, value: undefined }
          },
      ...
      expect(yield* Effect.flip(db.user.first())).toBeInstanceOf(
        StdbDecodeError,
      )
      expect(returnCount).toBe(1)
    }),
  ...
  it("validates all raw database keys before constructing a table handle", () => {
    const factory = StdbTesting.makeDbHandleFactory(LazyHandleModule)

    expect(() => factory.readwrite({ lazyA: {} } as never)).toThrow(
      StdbDbSchemaMismatchError,
    )
    try {
      factory.readwrite({ lazyA: {} } as never)
    } catch (error) {
      expect(error).toMatchObject({
        module: "lazy_handles",
        missingTables: ["lazyB"],
        availableTables: ["lazyA"],
      })
    }
  })

  it("constructs and memoizes only accessed table handles per invocation", () => {
    const factory = StdbTesting.makeDbHandleFactory(LazyHandleModule)
    let lazyAReads = 0
    let lazyBReads = 0
    const rawDb = Object.defineProperties(
      {},
      {
        lazyA: {
          enumerable: true,
          get: () => {
            lazyAReads = lazyAReads + 1
            return {}
          },
        },
        lazyB: {
    ...
    const readwrite = factory.readwrite(rawDb as never)
    expect(lazyAReads).toBe(0)
    expect(lazyBReads).toBe(0)
    const first = readwrite.lazyA
    expect(readwrite.lazyA).toBe(first)
    expect(lazyAReads).toBe(1)
    expect(lazyBReads).toBe(0)
    expect(Object.isFrozen(readwrite)).toBe(true)

    const readonly = factory.readonly(rawDb as never)
    expect(readonly.lazyA).not.toBe(first)
    expect("insert" in readonly.lazyA).toBe(false)
    expect(lazyAReads).toBe(2)
  })
})
