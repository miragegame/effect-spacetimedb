...
class CapturedTransportFixtureReadError extends Data.TaggedError(
  "CapturedTransportFixtureReadError",
)<{
...
const readCapturedTransport = (name: string) =>
  Effect.tryPromise({
    try: () => readCapturedJson(transportFixture(name)),
    catch: (cause) => new CapturedTransportFixtureReadError({ cause }),
  }).pipe(Effect.orDie)
