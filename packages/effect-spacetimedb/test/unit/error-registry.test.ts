...
describe("error codec", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const FirstErrors = StdbTesting.ContractError.define(
          MergeFirstError,
          MergeSharedError,
        )
        const SecondErrors = StdbTesting.ContractError.define(
          MergeSharedError,
          MergeSecondError,
        )
  ...
  it.effect("reads declared error tags without mutating schema classes", () =>
    Effect.gen(function* () {
      ...
      const IdentifierErrors =
        StdbTesting.ContractError.define(IdentifierOnlyError)
  ...
  it.effect("rejects duplicate and reserved declared error tags", () =>
    Effect.gen(function* () {
      ...
      expect(() =>
        StdbTesting.ContractError.define(DuplicateTagA, DuplicateTagB),
      ).toThrow(/duplicate error tag/i)
      expect(() =>
        StdbTesting.ContractError.define(RemoteRejectedError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbDeclaredErrorEncodingFailure),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbUniqueAlreadyExistsError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbAutoIncOverflowError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbNoSuchRowError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbScheduleDelayTooLongError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(StdbValueCodecError),
      ).toThrow(/reserved/i)
      expect(() =>
        StdbTesting.ContractError.define(ReducerGlobalRandomNotAllowedError),
      ).toThrow(/reserved/i)
    }),
  )
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const FirstErrors = StdbTesting.ContractError.define(SharedDeclaredOne)
        const SecondErrors = StdbTesting.ContractError.define(SharedDeclaredTwo)
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const HandSpreadErrors = StdbTesting.ContractError.define(
          MergeFirstError,
          MergeSharedError,
          MergeSecondError,
        )
  ...
  it.effect("supports typed declared-error picks from a shared registry", () =>
    Effect.gen(function* () {
      const RuntimePayloadErrors = StdbTesting.ContractError.define(
        NumericError,
        JsonBigIntError,
      )
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const PrototypeTagErrors = StdbTesting.ContractError.define(toString)
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const NumericErrors = StdbTesting.ContractError.define(NumericError)
        const BigIntErrors = StdbTesting.ContractError.define(JsonBigIntError)
    ...
    () =>
      Effect.gen(function* () {
        const MetadataErrors =
          StdbTesting.ContractError.define(MetadataRecordError)
        const encoded = yield* StdbTesting.ContractError.encodeString(
          MetadataErrors,
    ...
    (() => {
      ...
      return Effect.gen(function* () {
        const RuntimePayloadErrors =
          StdbTesting.ContractError.define(JsonPayloadError)
        const encoded = yield* StdbTesting.ContractError.encodeString(
