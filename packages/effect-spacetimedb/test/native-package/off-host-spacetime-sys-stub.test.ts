...
describe("spacetime sys testing stub", () => {
  ...
  it("routes host log levels to the matching console methods", () => {
    const calls: Array<string> = []
    const methods = ["error", "warn", "info", "debug", "trace", "log"] as const
    const originals = new Map(
      methods.map((method) => [method, globalThis.console[method]]),
    )
    try {
      ...
      for (const level of [0, 1, 2, 3, 4, 99]) {
        SpacetimeSysStub.console_log(level, "message")
      }
      expect(calls).toEqual(["error", "warn", "info", "debug", "trace", "log"])
    } finally {
      for (const method of methods) {
        Object.defineProperty(globalThis.console, method, {
          configurable: true,
